...
// Path: frontend/src/components/events/AccommodationListItem.js
...
const AccommodationListItem = ({ allocation, onDelete, index }) => {
    const [open, setOpen] = useState(false);
    ...
    const handleDelete = () => {
        setOpen(false);
        onDelete(allocation.allocation_id);
    };
    ...
    return (
        <List.Item style={style}>
            <Modal
                ...
                dimmer='blurring'
            >
                <Modal.Header>Delete {allocation.name}</Modal.Header>
                <Modal.Content>
                    <p>Are you sure you want to remove allocations of {allocation.name} from this Accommodation?</p>
                    <p>This action cannot be undone.</p>
                </Modal.Content>
                <Modal.Actions>
                    <Button negative onClick={() => setOpen(false)}>
            ...
            <Image avatar src={studentImage} size='tiny' />
            <List.Content>
                <List.Header as='a' onClick={() => navigate(`/students/${allocation.students_or_externals_id}/details`)}>{allocation.name}</List.Header>
                <List.Description>{allocation.email}</List.Description>
            </List.Content>
        </List.Item>
    );
}
